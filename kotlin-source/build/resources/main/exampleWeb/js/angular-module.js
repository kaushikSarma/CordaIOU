...
app.controller('DemoAppController', function($http, $location, $uibModal) {
    ...
    demoApp.settleIOU = (iou) =>
        $http.post(apiBaseURL + "settle-iou", {"linearID": iou['linearId']['id'], "amount": iou['value']})
        .then(
            (response) => {
                modalInstance.displayMessage(response.data);
                demoApp.getIOUs();
                demoApp.getMyIOUs();
            },
            (response) => {
    ...
    demoApp.getHistory = (iou) => {
        if (iou['showHistory'] === undefined || iou['showHistory'] === false) {
            iou['showHistory'] = true
            $http.get(apiBaseURL + "iou-history?linearID=" + iou['linearId']['id'])
            .then(
                (response) => {
                  iou['history'] = response.data.reverse()
