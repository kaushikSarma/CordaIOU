...
import com.example.flow.IOUSettleFlow
...
class SettleQuery {
    val linearID: String
    val amount: Int
    constructor(linearID: String, amount: Int) {
...
class ExampleApi(private val rpcOps: CordaRPCOps) {
    ...
    @Path("settle-iou")
    @Produces(MediaType.APPLICATION_JSON)
    fun settleIOU(requestData: String): Response{
        val mapper = ObjectMapper()
                .enable(JsonParser.Feature.ALLOW_UNQUOTED_FIELD_NAMES)
                .registerModule(KotlinModule())
        val query: SettleQuery = mapper.readValue(requestData, SettleQuery::class.java)
        print("Linear ID: ${query.linearID}\n")  // Template string concatenation
        if (query.amount <= 0 ) {
            return Response.status(BAD_REQUEST).entity("Settle 'amount' must be non-negative.\n").build()
        }
        if (query.linearID.length < 0) {
            return Response.status(BAD_REQUEST).entity("Linear ID is missing or has wrong format.\n").build()
        }
        return try {
            val signedTx = rpcOps.startTrackedFlow(IOUSettleFlow::Initiator, query.amount, query.linearID).returnValue.getOrThrow()
            Response.status(CREATED).entity("Transaction id ${signedTx.id} committed to ledger.\n").build()
        } catch (ex: Throwable) {
            logger.error(ex.message, ex)
            Response.status(BAD_REQUEST).entity(ex.message!!).build()
    ...
    @Path("iou-history")
    @Produces(MediaType.APPLICATION_JSON)
    fun getHistory(@QueryParam("linearID") linearID: String): Response {
        print("Transaction History for LinearID: $linearID")
        return try {
            val criteria = QueryCriteria.LinearStateQueryCriteria(linearId = listOf(UniqueIdentifier.fromString(linearID)))
                    .and(QueryCriteria.LinearStateQueryCriteria(status = Vault.StateStatus.UNCONSUMED).or(QueryCriteria.LinearStateQueryCriteria(status = Vault.StateStatus.CONSUMED)))
            val resultStates = rpcOps.vaultQueryBy<IOUState>(criteria).states
            Response.status(CREATED).entity(resultStates).build()
        } catch (ex: Throwable) {
            logger.error(ex.message, ex)
            Response.status(BAD_REQUEST).entity(ex.message!!).build()
        }
